...
find_fastest_mirror() {
    ...
    for mirror in "${!mirror_speeds[@]}"; do
        if [ ${mirror_speeds[$mirror]} -lt $best_time ]; then
            best_time=${mirror_speeds[$mirror]}
            best_mirror=$mirror
        fi
    done
    
    if [ -z "$best_mirror" ]; then
        echo -e "\e[31m错误：没有可用的镜像源，请检查网络连接\e[0m"
        return 1
    else
        echo -e "\n\e[32m最快镜像源：$best_mirror (${best_time}ms)\e[0m"
